...
# Shared SQL fragment for dot addressing (u.s.e.r@domain == user@domain).
# Resolves the dotted local part '%u' in domain '%d' (or the target of alias domain '%d')
# to exactly one mailbox. Yields a row only when the match is unambiguous over ALL mailboxes
# of the domain and the surviving mailbox is opted in. __DIR__ is replaced by "in" or "out".
DOT_MAILBOX_CORE="FROM mailbox
      JOIN domain ON domain.domain = mailbox.domain
      WHERE '%u' LIKE '%%.%%'
        AND '%u' NOT LIKE '%%+%%'
        AND domain.domain = COALESCE(
              (SELECT target_domain FROM alias_domain WHERE alias_domain = '%d' AND active = '1'), '%d')
        AND domain.active = '1'
        AND domain.dot_addressing___DIR__ = '1'
        AND mailbox.kind NOT REGEXP 'location|thing|group'
        AND REPLACE(mailbox.local_part, '.', '') = REPLACE('%u', '.', '')
        AND NOT EXISTS (SELECT 1 FROM alias WHERE address = '%s')
        AND NOT EXISTS (SELECT 1 FROM spamalias WHERE address = '%s')
      HAVING COUNT(*) = 1
        AND MIN(IFNULL(JSON_UNQUOTE(JSON_VALUE(mailbox.attributes, '\$.dot_addressing___DIR__')), '1')) = '1'"

# Shared SQL fragment for plus addressing policy (user+tag@domain).
# Yields a row when plus addressing is disabled for the base mailbox or its domain.
# __DIR__ is replaced by "in" (recipient) or "out" (sender).
PLUS_POLICY_CORE="FROM (SELECT 1) AS x
  WHERE '%u' LIKE '%%+%%'
    AND SUBSTRING_INDEX('%u', '+', 1) <> ''
    AND (
      EXISTS (SELECT 1 FROM domain
        WHERE plus_addressing___DIR__ = '0'
          AND domain = COALESCE(
            (SELECT target_domain FROM alias_domain WHERE alias_domain = '%d' AND active = '1'), '%d'))
      OR EXISTS (SELECT 1 FROM mailbox
        WHERE JSON_UNQUOTE(JSON_VALUE(attributes, '\$.plus_addressing___DIR__')) = '0'
          AND (
            username = CONCAT(SUBSTRING_INDEX('%u', '+', 1), '@', COALESCE(
              (SELECT target_domain FROM alias_domain WHERE alias_domain = '%d' AND active = '1'), '%d'))
            OR username IN (SELECT goto FROM alias
              WHERE address = CONCAT(SUBSTRING_INDEX('%u', '+', 1), '@', '%d')
                AND goto NOT LIKE '%%,%%')
          ))
    )"
...
query = SELECT IF(EXISTS(
  ...
  SELECT 1
  ${DOT_MAILBOX_CORE//__DIR__/in}
        AND MIN(mailbox.active) = '1'
        AND MIN(JSON_UNQUOTE(JSON_VALUE(mailbox.attributes, '$.tls_enforce_in'))) = '1'
  ), 'reject_plaintext_session', NULL) AS 'tls_enforce_in';
EOF

cat <<EOF > /opt/postfix/conf/sql/mysql_plus_addressing_rcpt_policy.cf
# Autogenerated by mailcow
user = ${DBUSER}
password = ${DBPASS}
hosts = unix:/var/run/mysqld/mysqld.sock
dbname = ${DBNAME}
query = SELECT '550 5.1.1 Plus addressing is disabled for this recipient' AS action
  ${PLUS_POLICY_CORE//__DIR__/in};
EOF

cat <<EOF > /opt/postfix/conf/sql/mysql_plus_addressing_sender_policy.cf
# Autogenerated by mailcow
user = ${DBUSER}
password = ${DBPASS}
hosts = unix:/var/run/mysqld/mysqld.sock
dbname = ${DBNAME}
query = SELECT '550 5.7.1 Plus addressing is disabled for this sender address' AS action
  ${PLUS_POLICY_CORE//__DIR__/out};
EOF

cat <<EOF > /opt/postfix/conf/sql/mysql_virtual_dot_addressing_maps.cf
# Autogenerated by mailcow
user = ${DBUSER}
password = ${DBPASS}
hosts = unix:/var/run/mysqld/mysqld.sock
dbname = ${DBNAME}
query = SELECT MIN(mailbox.username) AS goto
  ${DOT_MAILBOX_CORE//__DIR__/in}
        AND MIN(mailbox.active) IN ('1', '2');
EOF
...
query = SELECT goto FROM alias
  ...
  SELECT username FROM mailbox, alias_domain
    WHERE alias_domain.alias_domain = '%d'
      ...
      AND alias_domain.active='1'
  UNION
  SELECT MIN(mailbox.username)
  ${DOT_MAILBOX_CORE//__DIR__/out}
        AND MIN(mailbox.active) IN ('1', '2');
EOF
